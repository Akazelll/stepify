<x-app-layout>
    <div class="p-6 sm:p-10 max-w-6xl mx-auto">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-10">
            <div
                class="bg-white rounded-2xl p-6 shadow-[0_2px_12px_-4px_rgba(0,0,0,0.04)] border border-slate-100 flex items-center gap-5 transition-transform hover:-translate-y-1 duration-300">
        ...
        <div
            class="bg-white rounded-3xl shadow-[0_2px_20px_-4px_rgba(0,0,0,0.03)] border border-slate-100 overflow-hidden">
            <div class="px-8 py-6 border-b border-slate-100 flex justify-between items-center bg-white">
                ...
                <span
                    class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold bg-[#14B8A6]/10 text-[#14B8A6]">
            ...
            <div class="p-0">
                        ...
                        <table class="w-full text-left border-collapse">
                            <thead>
                                <tr class="bg-slate-50/50 border-b border-slate-100">
                                    <th
                                        class="px-8 py-4 text-xs font-semibold text-slate-400 uppercase tracking-wider w-16 text-center">
                                        #</th>
                                    <th class="px-8 py-4 text-xs font-semibold text-slate-400 uppercase tracking-wider">
                                        Kode Mata Kuliah</th>
                                    <th class="px-8 py-4 text-xs font-semibold text-slate-400 uppercase tracking-wider">
                                        Nama Mata Kuliah</th>
                                    <th
                                        class="px-8 py-4 text-xs font-semibold text-slate-400 uppercase tracking-wider text-right">
                                        Status Data</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-slate-50">
                                @foreach ($makulData as $index => $makul)
                                    <tr class="hover:bg-slate-50/70 transition-colors duration-150 group">
                                        <td class="px-8 py-5 text-sm text-slate-400 text-center font-medium">
                                            {{ $index + 1 }}</td>
                                        <td class="px-8 py-5">
                                            <span
                                                class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-mono font-semibold bg-[#0EA5E9]/10 text-[#0EA5E9] border border-[#0EA5E9]/20">
                                                {{ $makul['kdmk'] }}
                                            </span>
                                        </td>
                                        <td class="px-8 py-5">
                                        ...
                                        <td class="px-8 py-5 text-right">
                                            <span
                                                class="inline-flex items-center text-xs font-medium text-slate-400 group-hover:text-[#14B8A6] transition-colors duration-200">
                                                Tersedia
                                                <svg xmlns="http://www.w3.org/2000/svg"
                                                    class="h-4 w-4 ml-1 opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300"
                ...
                @else
                    <div class="py-16 flex flex-col items-center justify-center text-center">
                        <div
                            class="w-16 h-16 bg-[#EC4899]/10 text-[#EC4899] rounded-2xl flex items-center justify-center mb-4">
                        ...
                        <p class="text-sm text-slate-500 mt-2 max-w-sm">Tidak dapat terhubung ke Webservice. Silakan
                            coba <a href="{{ route('login') }}" class="text-[#0EA5E9] font-bold hover:underline">login
                                ulang</a> untuk memperbarui token akses Anda.</p>
                    </div>
                @endif
            </div>
        </div>
